...
glm::vec3 SunRadianceFromElevation(float elevationDeg)
{
    ...
    float airMass = 1.0f / (std::cos(zenithRad) +
        0.50572f * std::pow(96.07995f - zenithDeg, -1.6364f));
...
glm::vec3 AmbientSkyLight(float solarElevationDeg)
{
    solarElevationDeg = glm::clamp(solarElevationDeg, -18.0f, 90.0f);

    // --- Reference colors (linear RGB) ---
    // Deep night sky (no moon, clear)
    const glm::vec3 nightColor(0.02f, 0.03f, 0.06f);
    ...
    // --- Blend factors ---
    float nightToTwilight = glm::smoothstep(-18.0f, -6.0f, solarElevationDeg);
    float twilightToDay   = glm::smoothstep(-6.0f,  10.0f, solarElevationDeg);

    glm::vec3 color =
        glm::mix(nightColor, twilightColor, nightToTwilight);

    color =
        glm::mix(color, dayColor, twilightToDay);

    // Slight desaturation near horizon
    float horizonFade = glm::smoothstep(-2.0f, 10.0f, solarElevationDeg);
...
void Application::updateAnimatedVariables() {
    ...
    auto radiance = SunRadianceFromElevation(mSettings.sun.elevation);
    mSettings.sun.color = radiance;

    auto ambient_radiance = AmbientSkyLight(mSettings.sun.elevation);
