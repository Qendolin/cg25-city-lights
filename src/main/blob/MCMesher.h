...
class MCMesher {
public:
    using SDF = std::function<float(float,float,float)>;
private:
    const int edgeVertexIndices[12][2] = {{0, 1}, {1, 3}, {3, 2}, {2, 0}, {4, 5}, {5, 7},
                                          {7, 6}, {6, 4}, {0, 4}, {1, 5}, {3, 7}, {2, 6}};

    const int edgeTable[256] = {
        0x0,   0x109, 0x203, 0x30a, 0x80c, 0x905, 0xa0f, 0xb06, 0x406, 0x50f, 0x605, 0x70c, 0xc0a, 0xd03, 0xe09, 0xf00,
        0x190, 0x99,  0x393, 0x29a, 0x99c, 0x895, 0xb9f, 0xa96, 0x596, 0x49f, 0x795, 0x69c, 0xd9a, 0xc93, 0xf99, 0xe90,
        0x230, 0x339, 0x33,  0x13a, 0xa3c, 0xb35, 0x83f, 0x936, 0x636, 0x73f, 0x435, 0x53c, 0xe3a, 0xf33, 0xc39, 0xd30,
        0x3a0, 0x2a9, 0x1a3, 0xaa,  0xbac, 0xaa5, 0x9af, 0x8a6, 0x7a6, 0x6af, 0x5a5, 0x4ac, 0xfaa, 0xea3, 0xda9, 0xca0,
        ...
        0xca0, 0xda9, 0xea3, 0xfaa, 0x4ac, 0x5a5, 0x6af, 0x7a6, 0x8a6, 0x9af, 0xaa5, 0xbac, 0xaa,  0x1a3, 0x2a9, 0x3a0,
        0xd30, 0xc39, 0xf33, 0xe3a, 0x53c, 0x435, 0x73f, 0x636, 0x936, 0x83f, 0xb35, 0xa3c, 0x13a, 0x33,  0x339, 0x230,
        0xe90, 0xf99, 0xc93, 0xd9a, 0x69c, 0x795, 0x49f, 0x596, 0xa96, 0xb9f, 0x895, 0x99c, 0x29a, 0x393, 0x99,  0x190,
        0xf00, 0xe09, 0xd03, 0xc0a, 0x70c, 0x605, 0x50f, 0x406, 0xb06, 0xa0f, 0x905, 0x80c, 0x30a, 0x203, 0x109, 0x0,
    };
...
public:
    ...
    std::vector<glm::vec3> marchingCubes(const SDF& sdf);

private:
    std::vector<glm::vec3> getCellMeshVertices(float x0, float y0, float z0, float x1, float y1,
                                               float z1, const SDF &sdf);
    int getLookupIndex(const std::array<float, 8> &samples);
    glm::vec3 interpolate(const glm::vec3 &p0, const glm::vec3 &p1, float v0, float v1) const;
    std::array<glm::vec3, 8> buildCellVertices(float x0, float y0, float z0, float x1, float y1,
                                               float z1);
    std::array<float, 8> sampleGrid(const std::array<glm::vec3, 8> &, const SDF &sdf);
    std::array<glm::vec3, 12> getIntersections(int cubeLookupIndex,
                                               const std::array<glm::vec3, 8> &vertices,
                                               const std::array<float, 8>& samples);
    /* TEMP
    void tempWriteToObj(const std::string& path, const std::vector<glm::vec3>& vertices);
    float sdfUnitSphere(float x, float y, float z);
    */
};